...
impl UartReply {
    ...
    pub fn to_raw_packet(&self) -> [u8; 36] {
        ...
        match self {
            ...
            UartReply::Proxy { command_id, status, reply } => {
                let command_id = &u32::to_le_bytes(u32::from(*command_id))[..];
                let status = &i32::to_le_bytes(i32::from(*status))[..];
...
fn read_packet() -> Option<UartRequest> {
    ...
    if command_id.is_err() {
        warn!("Received invalid packet with command id: {}", raw_command_id);
        return None;
    }
    ...
    for i in 4..raw_packet.len() {
        raw_packet[i] = uart.read().unwrap();
    }
    ...
    if expected_checksum != computed_checksum {
        ...
        error!("Bad checksum {:x} vs {:x}", expected_checksum, computed_checksum);

        return None;
    }

    match command_id {
        CommandId::NoOperation => {
            Some(UartRequest::Simple {
                command_id
            })
        },
        CommandId::Proxy => {
            ...
            for i in 0..args.len() {
                args[i] = u64::from_le_bytes(raw_packet[12 + (i * core::mem::size_of::<u64>())..20 + (i * core::mem::size_of::<u64>())].try_into().unwrap());
            }
...
fn handle_packet(packet: UartRequest) -> UartReply {
    match packet {
        UartRequest::Simple { command_id } =>
        {
            match command_id {
                CommandId::NoOperation => return UartReply::Simple {
                    command_id,
                    status: Status::Ok
                },
                _ => {
